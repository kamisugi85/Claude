...
def test_screen_catalog_excludes_only_confident_matches():
    catalog = {
        ...
        "4": {"program_id": "4", "category": "回線", "備考": "SNS投稿は事前にご相談ください。"},
    }
    ...
    # program 1: explicit SNS OK; program 3: no restriction found (silence
    # alone is never held) -- excluded separately via the status keyword.
    assert overall_counts["likely_ok"] == 2
    assert overall_counts["likely_excluded"] == 1  # program 2 (risky category)
    assert overall_counts["needs_ai_or_human"] == 1  # program 4 (conditional wording)
